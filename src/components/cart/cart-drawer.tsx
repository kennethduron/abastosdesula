...
export function CartDrawer({
  ...
  return (
    ...
    >
      ...
      <Dialog.Portal>
        <Dialog.Backdrop className="fixed inset-0 z-[70] bg-brand-navy/45 backdrop-blur-[2px]" />
        <Dialog.Popup className="fixed inset-y-0 right-0 z-[80] flex h-dvh max-h-dvh w-full max-w-md flex-col bg-white pt-[env(safe-area-inset-top)] shadow-2xl sm:pt-0">
          <div className="flex items-start justify-between border-b border-slate-200 px-5 py-5">
            <div>
              ...
              <Dialog.Title className="mt-1 text-2xl font-extrabold text-brand-navy">
                Tu solicitud
              </Dialog.Title>
              {cart.items[0] && (
          ...
          <div className="max-h-[68dvh] overflow-y-auto overscroll-contain border-t border-slate-200 bg-white p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-[0_-12px_30px_rgba(7,26,51,0.06)]">
            {cart.items.length > 0 && (
              <>
