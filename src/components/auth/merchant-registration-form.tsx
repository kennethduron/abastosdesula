  ...
  return (
    ...
    >
      ...
      <fieldset className="grid gap-5 border-t border-slate-200 pt-6 sm:grid-cols-2">
        ...
        <Field label="Local o puesto (opcional)" error={errors.stall?.message}>
...
function Field({
  ...
  return (
    <label className="block text-sm font-bold text-brand-navy">
      {label}
      <span className="mt-2 block [&_input]:min-h-12 [&_input]:w-full [&_input]:rounded-xl [&_input]:border [&_input]:border-slate-300 [&_input]:px-4 [&_select]:min-h-12 [&_select]:w-full [&_select]:rounded-xl [&_select]:border [&_select]:border-slate-300 [&_select]:bg-white [&_select]:px-4">
        {children}
      </span>
      {error && (
